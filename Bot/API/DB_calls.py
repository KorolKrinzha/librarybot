import json
from API.DB_tools import DB_JSON, DB_COMMIT, DB_FETCH_ONE, DB_CHECK_EXISTENCE, DB_COMMIT_MULTIPLE, create_id
...
def add_quiz_choose(quiz_data):
    ...
    choose_insert_values = [(quiz_id, quiz_data['option_text'][i], quiz_data['option_correct'][i]) for i in
                            range(len(quiz_data['option_text']))]
...
def show_quiz_choose(quiz_id):
    ...
    quiz_data_choose = {
        ...
        'right_answer_reply': quiz_data[0]['right_answer_reply'],
        'wrong_answer_reply': quiz_data[0]['wrong_answer_reply'],
        'right_answers': [quiz_data_item['option_text'] for quiz_data_item in quiz_data if
                          quiz_data_item['option_correct']],
        'wrong_answers': [quiz_data_item['option_text'] for quiz_data_item in quiz_data if
                          not quiz_data_item['option_correct']]
